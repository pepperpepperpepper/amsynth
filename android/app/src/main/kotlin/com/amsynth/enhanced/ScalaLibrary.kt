 ...
 *   scala/scales.zip  — the scale files; an individual scale is streamed out on
...
object ScalaLibrary {
    ...
    /** The raw .scl text for an entry, streamed from the bundled zip, or null. */
    fun read(context: Context, entry: String): String? = runCatching {
        ZipInputStream(context.assets.open("scala/scales.zip")).use { zis ->
            var e = zis.nextEntry
            while (e != null) {
                if (e.name == entry) return@runCatching zis.readBytes().decodeToString()
                e = zis.nextEntry
            }
            null
        }
